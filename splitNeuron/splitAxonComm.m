...
%%% neuron into the area around the cell body, the commissure, and the rest
%%% of the neuron.
%%%
%%% Input Arguments
...
%%%      but with 'S* ' prepended to each, where the askterisk (*) is the segment name or number. The
%%%      segment number 1 refers to the segment with the green star in Figure 1 in the MTQuant documentation.

function fileOut = splitAxonComm(varargin)
...
fileOut = [folder,'\',name,'_splitComm',ext];
...
%%% Split the axon for each animal
dataC = [];
for count = 1:length(fileNames)
    dataFile = fileNames{count};  % Line Scan file
    acbFile = strrep(dataFile,'axon','acb');
    commFile = strrep(dataFile,'axon','comm');
    if exist(acbFile,'file') && exist(commFile,'file')
        ...
        scansIn = csvread(acbFile);
        gLineScan = scansIn(:,1);
        rLineScan = scansIn(:,2);
        currACB = calcSplitStats(gLineScan,rLineScan,length(gLineScan),Hs(count),rPeakTol,toUseRandRPeaks,rPeakCorr,toUseHalfMTs);
        
        scansIn = csvread(commFile);
        gLineScan = scansIn(:,1);
        rLineScan = scansIn(:,2);
        currComm = calcSplitStats(gLineScan,rLineScan,length(gLineScan),Hs(count),rPeakTol,toUseRandRPeaks,rPeakCorr,toUseHalfMTs);
        
        currC = [currACB,currComm,currC];
...
numPrefixes = (size(dataC,2)/6-2);
segVarNames = cellfun(@(x) strcat('S',num2str(x),'_',varNames(end-5:end)),[{'acb','comm'},mat2cell(1:numPrefixes,1,1*ones(1,numPrefixes))],'uniformoutput',false);
